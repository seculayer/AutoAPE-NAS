...
def _transform_data_cifar(using_normalize, using_crop, using_flip,
...
def load_cifar10_dataset(batch_size, split='train', using_normalize=True,
    ...
    dataset = dataset.map(
        _transform_data_cifar(using_normalize, using_crop, using_flip,
                                using_cutout, cutout_length),
        num_parallel_calls=tf.data.experimental.AUTOTUNE) ## 병렬처리
